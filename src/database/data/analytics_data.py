...
class AnalyticData:
    ...
    def interactions_data(
        ...
        with self.session:
            ...
            lending_result = self.session.query(func.count().label('interactions')).filter(
                LendingTransaction.dex_name == self.dex_name,
                LendingTransaction.wallet_address == self.wallet_address
            ).first()
    ...
    @property
    def volume_data(self) -> tuple[Optional[int], Optional[int], Optional[int], Optional[int]]:
        with self.session:
            ...
            lending_volume = self.session.query(func.sum(LendingTransaction.amount).label('total_volume')).filter(
                LendingTransaction.dex_name == self.dex_name,
                LendingTransaction.wallet_address == self.wallet_address
            ).first()
            return swap_volume[0], liquidity_volume[0], bridge_volume[0], lending_volume[0]
