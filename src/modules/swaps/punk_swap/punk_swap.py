...
from src.modules.swaps.punk_swap.utils.transaction_data import (
...
class PunkSwap(BaseSwap):
    def __init__(self, private_key: str, from_token: str, to_token: Union[str, List[str]],
        ...
        contract_address = contracts['punkswap']
        abi_name = abi_names['punkswap']
...
class PunkSwapLiquidity(BaseLiquidity):
    ...
    async def get_swap_instance(self, private_key: str, token: str, token2: str, amount: int) -> PunkSwap:
        return PunkSwap(private_key, token, token2, amount, False, 0, False)


class PunkSwapLiquidityRemove(BaseLiquidityRemove):
