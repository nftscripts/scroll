...
from src.modules.swaps.skydrome.utils.transaction_data import (
...
class SkyDromeSwap(BaseSwap):
    ...
    def create_swap_tx(self, from_token: str, to_token: str, contract: Contract, amount_out: int,
        ...
        return create_swap_tx(from_token, to_token, contract, from_token_address, to_token_address,
                              account_address, amount, web3)


class SkyDromeLiquidity(BaseLiquidity):
    ...
    async def get_swap_instance(self, private_key: str, token: str, token2: str, amount: int) -> SkyDromeSwap:
        return SkyDromeSwap(private_key, token, token2, amount, False, 0, False)


class SkyDromeLiquidityRemove(BaseLiquidityRemove):
